drop index manager_admin_user_username_uk_index on manager_admin_user;
...
alter table manager_admin_user comment '后台管理用户';

/*==============================================================*/
/* Index: manager_admin_user_username_uk_index                  */
/*==============================================================*/
create unique index manager_admin_user_username_uk_index on manager_admin_user
(
   username
);
...
(
   id                   bigint not null auto_increment comment '主键',
   manager_role_id      bigint not null comment '后台管理角色_id',
   manager_permission_resource_id bigint not null comment '后台管理权限资源_id',
   primary key (id)
);
...
(
   id                   bigint not null auto_increment comment '主键',
   manager_user_id      bigint not null comment '后台管理用户ID',
   manager_user_real_name varchar(128) comment '后台管理用户真实姓名',
   action_level         varchar(128) comment '操作级别(FATAL_1 : 致命,能影响到应用 , ERROR_2 : 错误,会影响正常功能, WARN_3 : 日常警告 ,INFO_5 : 日常记录)',
   action_type          varchar(12) comment '操作类型',
   action_log           varchar(1024) comment '操作日志(也用于可以存储异常栈信息,或者运行的sql)',
   acion_ip_address     varchar(64) comment '操作ip地址',
   ...
   aciton_total_time    datetime comment '总执行时间',
...
(
   id                   bigint not null auto_increment comment '主键',
   manager_admin_user_id bigint not null comment '后台管理用户_id',
   manager_role_id      bigint not null comment '后台管理角色_id',
   primary key (id)
)
charset = UTF8;
